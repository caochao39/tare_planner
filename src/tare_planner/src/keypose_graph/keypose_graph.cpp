...
int KeyposeGraph::AddNonKeyposeNode(int prev_node_index, const geometry_msgs::Point& new_node_position)
{
  if (prev_node_index < 0 || prev_node_index >= nodes_.size())
  {
    ROS_WARN_STREAM("KeyposeGraph::AddNode: prev_node_ind " << prev_node_index << " not in range [0, "
                                                            << nodes_.size() - 1 << "]");
    return -1;
  }
  ...
  if (closest_node_ind >= 0 && closest_node_ind < nodes_.size())
  {
    ...
    if (xy_dist < kAddNonKeyposeNodeMinDist && z_dist < 1.0)
    {
      next_node_index = closest_node_ind;
      dist_to_prev = misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(
          prev_node_position, nodes_[closest_node_ind].position_);
      node_exist = true;
    }
  }
  if (!node_exist)
  {
    ...
    dist_to_prev =
        misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(prev_node_position, new_node_position);
    KeyposeNode new_node(new_node_position, next_node_index, current_keypose_id_, false);
    new_node.SetCurrentKeyposePosition(current_keypose_position_);
    nodes_.push_back(new_node);
    std::vector<int> neighbors;
    graph_.push_back(neighbors);
  ...
  graph_[prev_node_index].push_back(next_node_index);
  graph_[next_node_index].push_back(prev_node_index);

  dist_[prev_node_index].push_back(dist_to_prev);
  dist_[next_node_index].push_back(dist_to_prev);

  return next_node_index;
}

void KeyposeGraph::AddToKeyposePath(const nav_msgs::Path& path)
{
  if (path.poses.size() < 2)
  {
    return;
  }
  int prev_node_index = static_cast<int>(path.poses.back().pose.orientation.w);

  if (prev_node_index >= 0 && prev_node_index < nodes_.size())
  {
    for (int i = path.poses.size() - 2; i >= 0; i--)
    {
      int next_node_index = AddNonKeyposeNode(prev_node_index, path.poses[i].pose.position);
      if (next_node_index < 0 || next_node_index >= nodes_.size())
      {
        ROS_WARN_STREAM("KeyposeGraph::AddToKeyposePath: next_node_index " << next_node_index << " out of range [0, "
                                                                           << nodes_.size() - 1 << "]");
        continue;
      }
  ...
  {
    ROS_WARN_STREAM("Cannot find node ind: " << prev_node_index << " in keypose graph");
  }
}

void KeyposeGraph::AddShortCutPath(const nav_msgs::Path& path)
{
  if (path.poses.size() < 2)
  {
    return;
  }
  int prev_node_index = static_cast<int>(path.poses.front().pose.orientation.w);
  int last_node_index = static_cast<int>(path.poses.back().pose.orientation.w);
  if (prev_node_index < 0 || prev_node_index >= nodes_.size())
  {
    double closest_node_dist = DBL_MAX;
    int closest_node_ind = 0;
    GetClosestNodeIndAndDistance(path.poses.front().pose.position, closest_node_ind, closest_node_dist);
    if (closest_node_dist < kAddNodeMinDist)
    {
      prev_node_index = closest_node_ind;
    }
    else
    {
      // ROS_WARN_STREAM(
      //     "KeyposeGraph::AddShortCutPath: cannot get the closest graph node to the path start, closest node dist: "
      //     << closest_node_dist);
      // std::cout << "path front: " << path.poses.front().pose.position.x << " " << path.poses.front().pose.position.y
      //           << " " << path.poses.front().pose.position.z << std::endl;
      // std::cout << "closest node ind: " << closest_node_ind << " " << nodes_[closest_node_ind].position_.x << " "
      //           << nodes_[closest_node_ind].position_.y << " " << nodes_[closest_node_ind].position_.z << std::endl;

      return;
    }
  }
  if (last_node_index < 0 || last_node_index >= nodes_.size())
  {
    double closest_node_dist = DBL_MAX;
    int closest_node_ind = 0;
    GetClosestNodeIndAndDistance(path.poses.back().pose.position, closest_node_ind, closest_node_dist);
    if (closest_node_dist < kAddNodeMinDist)
    {
      last_node_index = closest_node_ind;
    }
    else
    {
      // ROS_WARN_STREAM(
      //     "KeyposeGraph::AddShortCutPath: cannot get the closest graph node to the path end, closest node dist: "
      //     << closest_node_dist);
      // std::cout << "path back: " << path.poses.back().pose.position.x << " " << path.poses.back().pose.position.y <<
      // " "
      //           << path.poses.back().pose.position.z << std::endl;
      // std::cout << "closest node ind: " << closest_node_ind << " " << nodes_[closest_node_ind].position_.x << " "
      //           << nodes_[closest_node_ind].position_.y << " " << nodes_[closest_node_ind].position_.z << std::endl;
  ...
  // std::cout << "adding from: " << prev_node_index << " to " << last_node_index << std::endl;
  // std::cout << "before last node: " << last_node_index << " degrees: " << graph_[last_node_index].size() <<
  // std::endl;
  int next_node_index = prev_node_index;
  if (prev_node_index >= 0 && prev_node_index < nodes_.size() && last_node_index >= 0 &&
      last_node_index < nodes_.size())
  {
    for (int i = 1; i < path.poses.size() - 1; i++)
    {
      next_node_index = AddNonKeyposeNode(prev_node_index, path.poses[i].pose.position);
      if (next_node_index < 0 || next_node_index >= nodes_.size())
      {
        ROS_WARN_STREAM("KeyposeGraph::AddShortCutPath: next_node_index " << next_node_index << " out of range [0, "
                                                                          << nodes_.size() - 1 << "]");
        continue;
      }
      prev_node_index = next_node_index;
    ...
    geometry_msgs::Point prev_node_position = nodes_[prev_node_index].position_;
    geometry_msgs::Point last_node_position = nodes_[last_node_index].position_;
    double dist_to_prev =
        misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(prev_node_position, last_node_position);

    graph_[prev_node_index].push_back(last_node_index);
    graph_[last_node_index].push_back(prev_node_index);

    dist_[prev_node_index].push_back(dist_to_prev);
    dist_[last_node_index].push_back(dist_to_prev);

    // std::cout << "after last node: " << last_node_index << " degrees: " << graph_[last_node_index].size() <<
    // std::endl;
  }
  else
  {
    ...
    if (last_node_index < 0 || last_node_index >= nodes_.size())
    {
      ROS_WARN_STREAM("Cannot find node ind: " << last_node_index << " in keypose graph");
    }
  }
}
